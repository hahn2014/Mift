...
public class ShadowBox {
	...
	protected void update(Camera camera) {
		this.camera = camera;
		Matrix4f rotation = calculateCameraRotationMatrix(false);
	...
	protected void update(OverheadCamera camera) {
		this.overheadCamera = camera;
		Matrix4f rotation = calculateCameraRotationMatrix(true);
		Vector3f forwardVector = new Vector3f(Matrix4f.transform(rotation, FORWARD, null));
	...
	private Matrix4f calculateCameraRotationMatrix(boolean isOverhead) {
		...
		if (isOverhead == false) {
			...
			rotation.rotate((float) Math.toRadians(-overheadCamera.getYaw()), new Vector3f(0, 1, 0));
			rotation.rotate((float) Math.toRadians(-overheadCamera.getPitch()), new Vector3f(1, 0, 0));
		}
		return rotation;
	}
